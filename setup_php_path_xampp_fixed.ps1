...
$phpExists = Test-Path "$xamppPhpPath\php.exe"
...
if ($phpExists) {
    ...
    $phpInPath = $currentPath -like "*$xamppPhpPath*"
    ...
    $phpIni = "$xamppPhpPath\php.ini"
    
    if (Test-Path $phpIni) {
        Write-Host "  Found: $phpIni"  -ForegroundColor Green
    }
    else {
