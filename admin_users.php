...
if(isset($_GET['delete'])){
   ...
   if ($delete_id && $delete_id != $admin_id) {
       // remove user's uploaded image if exists
       $select_user_img = $conn->prepare("SELECT image FROM `users` WHERE id = ?");
       $select_user_img->execute([$delete_id]);
       $user_img = $select_user_img->fetch(PDO::FETCH_ASSOC);
       if(!empty($user_img['image']) && file_exists('uploaded_img/'.$user_img['image'])){
           @unlink('uploaded_img/'.$user_img['image']);
       }
